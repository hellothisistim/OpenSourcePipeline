...
class Volumes(list):
    ...
    def _mountedVolumes(self):
        ...
        # TODO: find correct platform name and mount points for Widows and Linux
    ...
    def enable(self, name, path):
        """Put the JSON token on this volume's root so OSP knows to pay attention to it and knows what name to use for it."""
        
        tokenFile = os.path.join( path, ospVolumeTokenName )
        
        # Check the provided volume path against the volumes known to be avalable.
        if path not in self._mountedVolumes():
            # TODO: compare common roots and offer a suggestion in the error message
            raise Exception(path, "is not a valid path to a volume's root directory.")
